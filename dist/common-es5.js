...
var CartService = /** @class */ (function () {
    ...
    CartService.prototype.getShops = function () {
        return this.shops;
    };
    CartService.prototype.clearShop = function () {
        this.shops = [];
        return this.shops;
    };
    CartService.prototype.getShipping = function () {
        return this.http.get('/assets/shipping.json'); /*.pipe(
            map(item => console.log(123, item)),
            tap(h => {
              const outcome = h ? `fetched` : `did not find`;
              console.log(`${outcome} hero id=${h}`);
            }),
            catchError(_ => console.log('fail'))
        )*/
    };
    CartService.prototype.searchShop = function (val) {
        if (!val.trim())
            return Object(rxjs__WEBPACK_IMPORTED_MODULE_3__["of"])([]);
        return this.http.get("/assets/shipping.json?name=" + val);
    };
